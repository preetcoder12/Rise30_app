...
exports.challengeRouter = router;
// Create a new custom challenge
router.post("/create", async (req, res) => {
    try {
        const { userId, name, description, type, category, duration, startDate, targetValue, unit, color, icon, } = req.body;
...
router.get("/:challengeId/analytics", async (req, res) => {
    try {
        ...
        for (let i = 0; i < weeks; i++) {
            const weekEntries = entries.filter((e) => e.dayNumber > i * 7 && e.dayNumber <= (i + 1) * 7);
            weeklyData.push({
                week: i + 1,
...
async function updateStreak(userId, challengeId) {
