...
router.get('/active/:userId', async (req, res) => {
    try {
        ...
        const currentDay = Math.min(Math.floor((Date.now() - startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1, 30);
        // Find today's entry by day number (more reliable than date comparison)
        const todayEntry = challenge.dailyTasks.find((entry) => entry.dayNumber === currentDay);
        const completedDays = challenge.dailyTasks.filter((e) => e.completed).length;
...
router.post('/log', async (req, res) => {
    try {
        ...
        const startDate = new Date(challenge.startDate);
        startDate.setHours(0, 0, 0, 0);
        const currentDayNumber = Math.min(Math.floor((now.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1, challenge.duration);
        // Find existing entry by day number (most reliable)
        let existingEntry = await supabaseClient_1.prisma.dailyEntry.findFirst({
            where: {
                userId,
                challengeId,
        ...
        if (existingEntry) {
            ...
            updatedEntry = await supabaseClient_1.prisma.dailyEntry.update({
            ...
            updatedEntry = await supabaseClient_1.prisma.dailyEntry.create({
                data: {
                    userId,
                    challengeId,
                    dayNumber: currentDayNumber,
        ...
        // Update streak
        const challengeWithStreak = await supabaseClient_1.prisma.challenge.findUnique({
            where: { id: challengeId },
            include: {
