...
router.get('/:userId/profile', async (req, res) => {
    try {
        ...
        const user = await supabaseClient_1.prisma.user.findUnique({
            where: { id: userId },
            include: {
                challenges: {
                    include: {
        ...
        const completedChallenges = user.challenges.filter(c => c.dailyTasks.every(d => d.completed)).length;
        const totalCompletedDays = user.challenges.reduce((acc, c) => acc + c.dailyTasks.filter(d => d.completed).length, 0);
        const longestStreak = Math.max(...user.challenges.map(c => c.streaks[0]?.length || 0), 0);
        res.json({
            success: true,
            profile: {
                id: user.id,
...
router.get('/:userId/analytics', async (req, res) => {
    try {
        ...
        const completedDays = challenges.reduce((acc, c) => acc + c.dailyTasks.filter(d => d.completed).length, 0);
        ...
        res.json({
            ...
            analytics: {
                ...
                completedChallenges: challenges.filter(c => c.dailyTasks.every(d => d.completed)).length,
                completionRate,
...
router.put('/:userId/profile', async (req, res) => {
    try {
        ...
        const user = await supabaseClient_1.prisma.user.update({
            where: { id: userId },
            data: {
...
router.get('/:userId/achievements', async (req, res) => {
    try {
        ...
        const completedChallenges = challenges.filter(c => c.dailyTasks.every(d => d.completed));
...
router.get('/search', async (req, res) => {
    try {
        ...
        const users = await supabaseClient_1.prisma.user.findMany({
            where: {
                OR: [
