...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        const challenges = await supabaseClient_1.prisma.challenge.findMany({
            where: { userId },
            include: {
                dailyTasks: {
        ...
        const formattedChallenges = challenges.map(c => {
            ...
            const currentDayNumber = Math.min(Math.floor((today.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1, c.duration);
...
router.post('/', async (req, res) => {
    try {
        const { userId, name, description, type, category, duration, targetValue, unit, color, icon } = req.body;
        ...
        const user = await supabaseClient_1.prisma.user.findUnique({ where: { id: userId } });
        if (!user) {
            // Auto-create user with a placeholder email if not exists
            await supabaseClient_1.prisma.user.create({
                data: {
                    id: userId,
        ...
        const challenge = await supabaseClient_1.prisma.challenge.create({
            data: {
                userId,
                name,
                description,
        ...
        await supabaseClient_1.prisma.dailyEntry.createMany({ data: dailyEntries });
        // Create initial streak
        await supabaseClient_1.prisma.streak.create({
            data: {
                id: (0, crypto_1.randomUUID)(),
                userId,
                challengeId: challenge.id,
...
router.get('/:challengeId', async (req, res) => {
    try {
        ...
        const challenge = await supabaseClient_1.prisma.challenge.findUnique({
        ...
        const currentDay = Math.min(Math.floor((Date.now() - new Date(challenge.startDate).getTime()) / (1000 * 60 * 60 * 24)) + 1, challenge.duration);
        res.json({
            success: true,
            challenge: {
                ...challenge,
...
router.post('/:challengeId/day/:dayNumber/toggle', async (req, res) => {
    try {
        ...
        const challengeVerify = await supabaseClient_1.prisma.challenge.findFirst({
            where: { id: challengeId, userId }
        });
        if (!challengeVerify) {
            return res.status(403).json({ success: false, error: 'Challenge not found or access denied' });
        }
        const updatedEntry = await supabaseClient_1.prisma.dailyEntry.updateMany({
            where: {
                challengeId,
                userId,
        ...
        if (challenge && challenge.streaks.length > 0) {
            ...
            await supabaseClient_1.prisma.streak.update({
...
router.post('/:challengeId/mark-today', async (req, res) => {
    try {
        ...
        const challenge = await supabaseClient_1.prisma.challenge.findFirst({
            where: { id: challengeId, userId }
        });
        if (!challenge) {
        ...
        const offsetMs = tzOffset ? parseOffset(tzOffset) : 0;
        ...
        const currentDay = Math.min(Math.floor((localNow.getTime() - localStart.getTime()) / (1000 * 60 * 60 * 24)) + 1, challenge.duration);
        await supabaseClient_1.prisma.dailyEntry.updateMany({
            where: {
                challengeId,
                userId,
...
router.put('/:challengeId', async (req, res) => {
    try {
        ...
        const updatedChallenge = await supabaseClient_1.prisma.challenge.update({
            where: { id: challengeId },
            data: {
        ...
        if (durationInt !== undefined && durationInt !== challenge.duration) {
            if (durationInt < challenge.duration) {
                ...
                await supabaseClient_1.prisma.dailyEntry.deleteMany({
                    where: {
                        challengeId,
                ...
                if (newEntries.length > 0) {
                    await supabaseClient_1.prisma.dailyEntry.createMany({ data: newEntries });
                }
            }
            // Update end date
            const newEndDate = new Date(challenge.startDate);
            newEndDate.setDate(newEndDate.getDate() + durationInt);
            await supabaseClient_1.prisma.challenge.update({
                where: { id: challengeId },
...
router.delete('/:challengeId', async (req, res) => {
    try {
        ...
        const challenge = await supabaseClient_1.prisma.challenge.findFirst({
            where: { id: challengeId, userId: userId }
        });
        if (!challenge) {
            return res.status(404).json({ success: false, error: 'Challenge not found or access denied' });
        }
        // Delete related records manually to simulate Cascade delete
        await supabaseClient_1.prisma.dailyEntry.deleteMany({ where: { challengeId } });
        await supabaseClient_1.prisma.waterEntry.deleteMany({ where: { challengeId } });
        await supabaseClient_1.prisma.streak.deleteMany({ where: { challengeId } });
        await supabaseClient_1.prisma.challenge.delete({
            where: { id: challengeId }
        });
